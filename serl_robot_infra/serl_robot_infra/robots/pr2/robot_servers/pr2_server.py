...
from serl_robot_infra.robots.pr2.robot_servers.pr2_gripper_server import (
    PR2GripperServer,
)
from serl_robot_infra.robots.pr2.robot_servers.spacenav_server import SpacenavServer
from serl_robot_infra.robots.pr2.robot_servers.camera_server import CameraServer
...
flags.DEFINE_list(
    "reset_joint_target",
    [RESET_JOINT_POSITIONS[joint] for joint in ALL_JOINT_NAMES],
...
class PR2Server(object):
    ...
    def __init__(self, robot_ip, ros_pkg_name, reset_joint_target):
        ...
        self.pub_pose = rospy.Publisher(
            "/r_arm_cartesian_impedance_controller/equilibrium_pose",
            PoseStamped,
            queue_size=10,
        )
        ...
        self.sub_robot_state = rospy.Subscriber(
            "/r_arm_cartesian_impedance_controller/controller_state",
            ControllerState,
    ...
    def reset_joint(self):
        ...
        reset_joint_msg.name = ALL_JOINT_NAMES
        ...
        count = 0
        time.sleep(1)
        # get the joint positions of R_ARM_JOINT_NAMES
        joint_indices = [self.joint_names.index(joint) for joint in ALL_JOINT_NAMES]
        while not np.allclose(
            ...
            self.pub_joint.publish(reset_joint_msg)
            time.sleep(1)
            count += 1
            if count > 30:
                rospy.loginfo("joint reset TIMEOUT")
                break
...
def main(_):
    ...
    reconf_client = ReconfClient(
        "r_arm_cartesian_impedance_controllerdynamic_reconfigure_compliance_param_node"
    )
